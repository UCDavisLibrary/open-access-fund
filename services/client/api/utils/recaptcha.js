...
async function validateRecaptcha(req, res, next){
  ...
  try {
    ...
    if (!response.tokenProperties.valid) {
      logger.info({corkTraceId: req.corkTraceId, message: 'Invalid recaptcha token', details: response.tokenProperties.invalidReason});
    ...
    if ( response.tokenProperties.action !== config.recaptcha.submissionAction.value ) {
      logger.info({corkTraceId: req.corkTraceId, message: 'Recaptcha action mismatch', details: {expected: config.recaptcha.submissionAction.value, got: response.tokenProperties.action }});
    ...
    if ( response.riskAnalysis.score < config.recaptcha.scoreThreshold.value ) {
      logger.info({corkTraceId: req.corkTraceId, message: 'Recaptcha score too low', details: response.riskAnalysis});
      return res.status(401).json({error: 'Recaptcha score too low'});
    }

    logger.info({corkTraceId: req.corkTraceId, message: 'Recaptcha passed', details: response.riskAnalysis});
